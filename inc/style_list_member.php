...
<td colspan="2" style='border:none;'><!-- <select  name="cls1" id="cls1" data-value="<?=$cls1?>" data-first-title="选择分类"  ></select> -->
...
<input type="text" name="keys" lay-verify="title" autocomplete="off" placeholder="请输入款号或款名" class="layui-input" style='width:240px; float:left;margin:0px 25px 0px 15px;' value="<?php if( $keys ){ echo $keys; }?>" >
<button type="submit" class="layui-btn"  onclick="look()">提交查询</button>
<a href='./style_list_member.php' class="layui-btn" onclick="look()" >返回</a>
</td>
</form>
</tr>
<form method='post' action="style_all.php" id='form2' >
<?php
...
{
if( $rs['style_thumb'] ){
    $best64 = $rs['best64']?'imagespath/':'';
    $style_thumb = urldecode($rs['style_thumb']);
    $img_url = "http://imageserver.echao.com/".$best64.$style_thumb;
}else{
    $img_url = "../images/timg.jpg";
}
?>
<tr align="center">
  <td><img  class='img_hover' src=" <?=$img_url?> " style="width:60px;height:60px"></td>
  <td><?=$rs['style_name']?></td>
  <td><?=$rs['style_no']?></td>
  <td style="line-height:30px">-</td>
  <td> - </td>
  <td> 库存 (<?=$rs['stocks']?>)</td>
  <td><a href="#"  class="layui-btn" >现货列表</a> <?php
//如果登录为主管理帐号uid=1 则没有上架下架
  if( $_SESSION['uid'] > 1 ){ 
    $past_id = "select past_id from e_goods_sylte  where displays = '0' and adduid=".$_SESSION['uid'];
    ...
    if( @in_array($array, $past) ){
        echo "<a href='style_all.php?del=".$rs['id']."' onclick='look()' class='layui-btn layui-btn-danger' >下架</a>"; 
     }else{
       echo "<a href='style_all.php?add=".$rs['id']."'  onclick='look()' class='layui-btn' >上架</a>"; 
     } 
  } 
   ?></td>
</tr>
