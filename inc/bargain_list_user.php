...
if ($title) {
    $sql .= " and a.title  like '%$title%' order by a.id desc";
} else {
    ...
    <table class="layui-table" lay-skin="line">
        <form action=''>
            <tr style=' background:#ffffff;'>
                <td style='text-align:center;border:none;' colspan="12">
                    ...
                    <a href='./bargain_list_user.php' onclick="show()" class="layui-btn"> 返 回 </a></td>
            </tr>
        </form>
        ...
        while ($rs = mysql_fetch_array($results_date)) {
           ...
           // $url = urlencode("http://weixin.echao.com/web_activity/bargain/index.html?userid=" . $rs['userid'] . "&actid=" . $rs['id']);
            ...
            <tr align="center">
                ...
                <td><img src="<?php
                        if( $rs['image_logo'] && is_null(json_decode($rs['image_logo']))  ){
                          echo   $rs['image_logo'];
                        }else{
                ...
                <td width="350">
                    <?php
                    if(  $rs['status'] == 0 ){
                        echo "<a class='layui-btn layui-btn-danger' onclick='openpage( ".$rs['id'].",".$rs['status']." )' title=\"审核\">未审核</a>";
                    }else{
                        echo "<a class=\"layui-btn\"  onclick='openpage( ".$rs['id'].",".$rs['status']." )' title=\"审核\">已审核</a>";
                    }
                    ?>
                    <a class="layui-btn" onclick="openpage1( <?= $rs['id'] ?> )">参加用户</a>
                    <a class="layui-btn layui-btn-danger" onclick="openpage3( <?= $rs['id'] ?> )" title="删除"><i
        ...
        <tr align='center' style=' background:#ffffff;'>
            <td colspan="12"> <?php echo $pageNavi; ?> 共 <span
...
<script>
    ...
    //打开修改页面的弹出层
    function openpage(id ,status) {
        var editid = id;
        layer.confirm('确定要修改此活动状态吗？', {
            btn: ['确认', '取消'] //按钮
        }, function () {
            $.ajax({
                type: "POST",
                url: "bargain_user_edit.php",
                data: "id=" + editid+"&status="+status,
