...
if ($categorytype == 0) {
    ...
    //全部数据
    $area = fetchAll("select id,catename,webid,pid,adduid,box_default_ico from e_goods_category  where  adduid in($domain$uid) ");
...
<fieldset class="layui-elem-field layui-field-title" style="margin: 20px;">
    ...
    <table class="layui-table" lay-even="" lay-skin="nob" width='500'>
        ...
        <tr>
            ...
            <td colspan='2'><font size='4'><b>倍 率</b></font><font size='1' color='#1AA094'>（*双击可修改*）</font></td>
        ...
        <tr>
            ...
            <td align='left' width="300">
                <div class="am-u-sm-8  spanc1" id="p01" lang="<?= $uid ?>"></div>
                <!-- <?php echo show_zk_s($uid, 'p0', '') * 100 ?> -->
                <div class="am-u-sm-8 editable spanc1" id="p01"
                     lang="<?= $uid ?>"><?php echo show_zk_s($uid, 'p01', '') * 100 ?></div>
                <div class="am-u-sm-8 editable spanc1" id="p02"
                     lang="<?= $uid ?>"><?php echo show_zk_s($uid, 'p02', '') * 100 ?></div>
                <div class="am-u-sm-8 editable spanc1" id="p03"
                     lang="<?= $uid ?>"><?php echo show_zk_s($uid, 'p03', '') * 100 ?></div>
                <div class="am-u-sm-8 editable spanc1" id="p04"
                     lang="<?= $uid ?>"><?php echo show_zk_s($uid, 'p04', '') * 100 ?></div>
                <div class="am-u-sm-8 editable spanc1" id="p05"
                     lang="<?= $uid ?>"><?php echo show_zk_s($uid, 'p05', '') * 100 ?></div>
                <div class="am-u-sm-8 editable spanc1" id="p06"
                     lang="<?= $uid ?>"><?php echo show_zk_s($uid, 'p06', '') * 100 ?></div>
                <div class="am-u-sm-8 editable spanc1" id="p07"
                     lang="<?= $uid ?>"><?php echo show_zk_s($uid, 'p07', '') * 100 ?></div>
                <div class="am-u-sm-8 editable spanc1" id="p08"
                     lang="<?= $uid ?>"><?php echo show_zk_s($uid, 'p08', '') * 100 ?></div>
                <div class="am-u-sm-8 editable spanc1" id="p09"
                     lang="<?= $uid ?>"><?php echo show_zk_s($uid, 'p09', '') * 100 ?></div>
                <div class="am-u-sm-8 editable spanc1" id="p10"
                     lang="<?= $uid ?>"><?php echo show_zk_s($uid, 'p10', '') * 100 ?></div>
                <div class="am-u-sm-8 editable spanc1" id="p11"
                     lang="<?= $uid ?>"><?php echo show_zk_s($uid, 'p11', '') * 100 ?></div>
            </td>
        ...
        foreach ($tree as $v) {
            ...
            <tr>
                <td></td>
                <td align='left' style='height:24px;'><?php if ($v['lev'] == 1) {
                        echo '<b>' . str_repeat('　　', $v['lev'] - 1), $v['catename'] . '　:' . '</b>';
                    } else {
                        echo str_repeat('　　', $v['lev'] - 1), $v['catename'] . '　:';
                    } ?></td>
                <td>
                    <span id="p<?= $v['id'] ?>" class="editable spanc1" lang="<?= $uid ?>"><?php if ($v['lev'] == 2) {
                            echo show_zk_s($uid, 'p' . $v['id'], '') * 100;
                        } ?></span>
                </td>
                <td><? if ($uid == 1) { ?><?php if ($v['lev'] == 2) {
                        echo show_zk($uid, 'p' . $v['id'], $domain);
                    } ?><? } ?></td>
            </tr>
            <?php
