...
{
    $clsid = 'p0';
	if($size < 0.3) $clsid = 'p01';
	if($size >= 0.3 && $size < 0.5) $clsid = 'p01';
	if($size < 0.3) $clsid = 'p01';
	...
	if($size*1 < 0.3)
	 ...
	 if($Size*1 >= 0.3 && $Size*1 < 0.5)
	 {
	    $clsid = 'p02';
		
	 }
	 
	 if($Size*1 >= 0.5 && $Size*1 < 1)
	 {
	    $clsid = 'p03';
	...
	function pages($sql_date,$s,$page_get,$pl_item_total=10,$self_tmp='')
	{
		...
		if($numrows_date!=0)
		{
			...
			if ($s>=1) { // bypass PREV link if s is 0
				...
				if($s > 1)
				{
					$pageNavi.="<a href='$self_tmp?s=0&$page_get' target='_self'  class='frist_page'>首页</a>&nbsp&nbsp; ";
					$pageNavi.="<a href='$self_tmp?s=$prevs&$page_get' target='_self' class='up_page' >上一页</a>&nbsp&nbsp; ";
				}
				else
				{
					$pageNavi.="<a href=\"javascript:\" target='_self' class='frist_page' >首页</a>&nbsp&nbsp; ";
					$pageNavi.="<a href=\"javascript:\" target='_self'  class='up_page' >上一页</a>&nbsp&nbsp; ";
				}
			}
			...
			//echo "Page#: $tmp_page<br>"; 
				...
				if($pages>1)
				{
					   ...
					   for($c=1;$c<=$pages;$c++)
					   {
							...
							if($tmp_page==$c)
							{
								$pageNavi_f="<font color=red  id='this_page' ><b>";
								$pageNavi_b="</b></font>&nbsp;&nbsp;";
							}else{
								$pageNavi_f="<a href='$self_tmp?s=$s_tmp&$page_get' target='_self'  class='a_page' >";
								$pageNavi_b="</a>&nbsp;&nbsp;";
							}
					   ...
					   {  
						  $pageNavi.="<a href='$self_tmp?s=$news&$page_get' target='_self' class='next_page' >下一页</a>&nbsp;&nbsp;";
						  $pageNavi.="<a href='$self_tmp?s=$s_tmp&$page_get' target='_self' class='end_page' >尾页</a>";
					   }
					   else
					   {
					      $pageNavi.="<a href=\"javascript:\" target='_self' class='next_page' >下一页</a>&nbsp;&nbsp;";
						  $pageNavi.="<a href=\"javascript:\" target='_self' class='end_page' >尾页</a>";
					   }
